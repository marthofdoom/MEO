...
- **Multiple different Echo shares now stack on you at once.** In v1.0.9 each character
  received only the single *strongest* incoming Echo share. Now every distinct share
  applies — two followers wearing, say, Echo + Fortify Carry and Echo + Fortify Health
  both benefit you simultaneously (same effect from two sources still de-duplicates to
  the stronger, so nothing double-counts).
- Under the hood: each recipient gets a dedicated ability from a small pool, so distinct
  shares never collide and removing one never disturbs another. Requires the updated
  `MEO.esp` (regenerated by the installer) — no save changes.
