...
### TRAP: a Synthesis GROUP named "MEO" outputs `MEO.esp` and destroys the mod

Synthesis names each group's output plugin after the GROUP (`<Group>.esp`); our
`SetTypicalOpen("MEO - Patch.esp")` only applies to standalone runs. A user who
names their Synthesis group **"MEO"** gets an output plugin `MEO.esp` that
overwrites (vanilla) or shadows (MO2) the real MEO.esp. Signature (a real
report): xEdit shows "MEO.esp" containing only an AVEnchanting override + a few
perks; in-game the DLL can't find the Gem Pouch SPEL (0x803) → pouch/menu dead
("the mod stopped working", "spells disappeared from MEO.esp"); the Synthesis
run reports **success** (referencing MEO.esp-local FormKeys from a patch mod
whose own ModKey is MEO.esp is legal for Mutagen). Guarded now (RunPatch refuses
if the output is `MEO.esp`; `ApplyPatch` resolves perk 0x810 to prove the real
plugin is loaded, not just that the NAME is present — presence ≠ integrity).
User fix: rename the group, reinstall MEO.esp, re-run.

### Empty settings page = Synthesis GUI / .NET mismatch, NOT our code

`SetAutogeneratedSettings` is correct (verified at SynthesisPipeline source
0.36.5; public fields + `[SynthesisSettingName]` render). If the settings page
is BLANK while the patch still runs, it's upstream (issue #547 class): Synthesis
rewrites our TargetFramework to net10 at build (Mutagen ≥0.54), and a GUI older
than the net10-capable 0.36.x builds can't load the patcher assembly in-process
to reflect its settings. Fix: **update Synthesis to ≥0.36.5**. Patcher-side
health check: `dotnet MEO.Synthesis.dll settings-query` prints the reflected
config. Hand-edit off-switch (no GUI): the run log prints
`No settings file found. Using defaults. Path: <abs path>` — create that
`settings.json` with `{ "MintMultiEffectRecipes": false }`; it's honored next
run (Newtonsoft populates public fields; missing file = defaults). Standalone:
`--no-mint-riders`.

### String encoding: default is Windows-1252

`BuildCalibrationLoadOrder` passes `BinaryReadParameters` with
`StringsParam.NonLocalizedEncodingOverride = MutagenEncoding._utf8_1252`
(namespaces `Mutagen.Bethesda.Plugins.Binary.Parameters` + `.Strings` +
`.Strings.DI`). Without it, embedded strings decode as 1252 and a UTF-8 plugin
(e.g. a Japanese Unslaad) garbles — cosmetic in logs, but it would bake mojibake
into phase-3 minted gem NAMES. `_utf8_1252` = UTF-8 first, 1252 fallback; ASCII
is byte-identical so English-list output is unchanged (verified: Authoria
calibration identical before/after). CJK plugins authored in Shift-JIS/CP932
(not UTF-8) would need `_utf8_932` — a per-game-language choice to settle before
phase-3 ships wide. Encoding is near-inert to matching: conversion/minted-slot
keys are FormKey/filename-based (MintGid never reads Name), and the one
name-keyed surface — the m28 kin-ladder root match — compares rungs that decode
from the same plugin, so a shift moves both sides identically. EditorIDs are
untouched (they decode under `NonTranslatedEncodingOverride`, which we do not
set).
