...
version and Remove+AddSpell in the migration. Directly relevant to MEO:
gem magnitude effects on worn gear are exactly this record class.
...
- PO3 player events: `RegisterForWeaponHit` → `OnWeaponHit(target, source,
  ...
  **CRITICAL: the receiving script must extend ObjectReference,
  ...
  delivered** — this silently zeroed weapon XP for a whole MRO test pass.
  ...
  (MEO earns gem XP from kills — `OnActorKilled` on such an AME is the
  natural host.) Note `RegisterForHitEventEx` is for hits ON the
  registered reference — not for the player's outgoing hits.
