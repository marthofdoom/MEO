...
How our Synthesis patcher (MEO.Synthesis) is built, distributed, kept building
...
writes MEO's calibration output in addition to record patches.
...
MEO.Synthesis (`installer/MEO.Synthesis/MEO.Synthesis.csproj`, C#/Mutagen) is a
...
- **Fix**: bump **all three together** — `TargetFramework` +
  ...
  **MEO.Synthesis is still pinned net9.0 / 0.36.5 / 0.54.2 — the same latent
...
- **Verifying without the matching SDK**: you can't run the new TFM locally
  ...
  For MEO specifically the shared `MEO.Installer/Commands.cs` must compile
  under the bumped Mutagen too (it's the same record model by design).
- Sibling repos: MFO and MAO carry the same patcher pattern; when one breaks
  on a Synthesis bump, fix all three (MFO is already on net10.0/0.36.6/0.54.4;
  MAO was still net9.0/0.36.5/0.54.2 as of 2026-08-11).

## 4. How users add, run, and update the patcher

- **By git URL**: Git Repository Patcher → paste
  `https://github.com/marthofdoom/MEO` → pick `MEO.Synthesis` from the project
  dropdown. Works whether or not we're listed in the browser.
- **From the in-app browser**: same thing, pre-filled — but only
  registry-listed repos appear (section 5). MEO is listed.
...
2. A `.csproj` referencing **`Mutagen.Bethesda.Synthesis`** — pushed, and
   **indexed by GitHub's dependency graph**. If the graph is disabled or
   ...
   `gh api repos/marthofdoom/MEO/dependency-graph/sbom` — a 404 means the
...
## 6. This repo (MEO) — status + checklist

Status verified 2026-08-11:

- [x] Repo public (`marthofdoom/MEO`)
- [x] `installer/MEO.Synthesis/MEO.Synthesis.csproj` references
      `Mutagen.Bethesda.Synthesis`; dependency graph indexed (SBOM lists it)
- [x] `SynthesisMeta.json` present next to the csproj
- [x] **LISTED** in the registry (`marthofdoom/MEO`, nickname "marth
      Enchanting Overhaul (MEO)") — confirmed in the live
